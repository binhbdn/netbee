        ...
        <div class="card-content" style="grid-template-columns: 3fr 6fr 1fr;">
            <div style="margin: 0 auto;">
                <a :href="`/tin-tuyen-sinh/${job.id}/${ChangeToSlug(job.title)}`">
                    <img v-lazy="`/uploads/users/avatars/${job.avatar}`" :alt="`${job.avatar}`" width="55" height="55">
                </a>
            </div>
            <div class="remove-border-right">
                <div class="item-name">
                    <p class="item-company mb-0"><i class="fad fa-building"></i> <span class="company-name"  data-toggle="tooltip" data-placement="right" :title="`${job.name}`"> {{ job.name }}</span></p>
                </div>
                ...
                <div class="item-quantity ">
                    <p class="delivery-date" style="font-size: 13px;"><i class="fad fa-funnel-dollar"></i> {{ job.salary_start }}{{ job.currency }} ~ {{ job.salary_end }}{{ job.currency }}</p>
                </div>
            </div>
        </div>
    </div>
